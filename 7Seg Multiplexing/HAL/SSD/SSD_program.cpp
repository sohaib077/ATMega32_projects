...
u8 SSD_u8EnableSsd(SSD *Copy_SSDPtr) {
    ...
    Locale_u8ErrorState += DIO_u8SetPinDirection(Copy_SSDPtr->enablePort, Copy_SSDPtr->enablePin,
                                                 DIO_u8PIN_OUTPUT);

    Locale_u8ErrorState += DIO_u8SetPinValue(Copy_SSDPtr->enablePort, Copy_SSDPtr->enablePin,
                                             Locale_u8PinValue);
    if (Locale_u8ErrorState == 0) return 0;
    return 1;
};
